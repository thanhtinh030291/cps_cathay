            ...
            if($key == 'default'){
                foreach ($value as $key2 => $value2) {
                    ...
                    if($value2['PAYMENT_METHOD'] == 'CH'){
                        ...
                        $tw['payment']['DESC'] = "Thanh toán bồi thường hộ CATHAY cho số {$value2['CL_NO']}  (Hợp đồng BH số {$value2['POCY_REF_NO']}), Nhận tiền mặt tại {$value2['BANK_NAME']}, {$value2['BANK_CITY']}, {$value2['BANK_BRANCH']} bằng CMND số {$ppDay} của {$value2['BENEFICIARY_NAME']}, ngày cấp: {$value2['PP_DATE']}, nơi cấp: {$value2['PP_PLACE']}";
                    }else{
                        $tw['payment']['DESC'] = "Thanh toán bồi thường hộ CATHAY cho số {$value2['CL_NO']}  (Hợp đồng BH số {$value2['POCY_REF_NO']}), Claim payment for CATHAY";
                    }
                ...
                $tw['payment']['DESC'] = "Thanh toán bồi thường hộ CATHAY cho số {$all_cl_no}, Claim payment for CATHAY";
                $html = $this->load->view('pdf/bank_request_form', $tw, true);
                $mpdf->AddPage();
                $mpdf->WriteHTML($html);
            }
